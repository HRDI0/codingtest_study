...
# T의 2～m+1번 문자와 P의 1～m번 문자를 비교한다면 다시 최대 m번의 연산이 수행된다. 매칭은 T의 n-m+1번 문자에서까지 시작할 수 있으므로,
#  이러한 방식으로 진행한다면 O( (n-m+1) × m ) = O(nm) 의 시간복잡도를 갖는 알고리즘이 된다.

# 더 좋은 방법은 없을까? 물론 있다. 위에 제시된 예에서, 
# T[7] ≠ P[7] 이므로 T의 1번 문자에서 시작하는 매칭이 실패임을 알게 된 순간으로 돌아가자. 
# 이때 우리는 매칭이 실패라는 사실에서, T[7] ≠ P[7] 라는 정보만을 얻은 것이 아니다. 
# 오히려 i=1…6에 대해 T[i] = P[i] 라고 하는 귀중한 정보를 얻지 않았는가? 이 정보를 십분 활용하면, 
# O(n)의 시간복잡도 내에 문자열 매칭 문제를 풀어내는 알고리즘을 설계할 수 있다.
...
# 입력
# 첫째 줄에 문자열 T가, 둘째 줄에 문자열 P가 주어진다. T와 P의 길이 n, m은 1이상 100만 이하이고, 
...
def getPi(input_str):
    ...
    begin = 1
    matched = 0
    #kmp 알고리즘으로 접두사와 접미사가 일치하는 부분을 찾는다.
    #이때 begin이 0인경우 비교 문자가 한개이므로 접두사, 접미사가 없다. 그렇기 때문에 begin은 1부터 시작한다.

    #begin은 기준 문자열에서 비교시작하는 위치 matched는 비교할 문자열의 길이에 해당하기 때문에
    #두 값을 합한 길이가 기준문자열을 초과하면 없는 리스트 요소 참조가 된다.
    while begin + matched < m:
        if input_str[begin+matched] == input_str[matched]: #ABXAB 기준,  AB 에서 A를 접두사 접미사로 확인한다.
            matched += 1                                   #만약 같다면 접두사를 A -> AB로 늘리고

            #ABXAB 를 예로 ABXA에서 A가 접두사 접미사이므로 pi[3]에 일치하는 접두사,접미사 길이 (즉, matched)만큼 기록한다.
            pi[(begin+matched) - 1] = matched
        else:                                              #AB 기준, A는 접두사이지만 접미사가 아니기 때문에
            if matched == 0:                               #AB(XAB) -> ABX(AB) -> ABXA(B) 로 기준 문자열을 늘려간다. 
                begin += 1
            else:                                          #접두사이지만 접미사는 아님에도 matched가 0이 아니라면
                begin += (matched - pi[matched-1])          #이전 문자열 길이만큼 접미사였다는 의미이므로 
                matched = pi[matched - 1]                   #기준 문자열에서 일치했던 길이만큼 뛰어넘고
    return pi                                               #비교한 문자열의 길이를 이전으로 되돌린다. ex) AB -> A
...
def kmp(H,N):
    ...
    begin = 0
    matched = 0
                                                #기준 문자열이 자기자신이 아닌것만 다르고 전체적인 비교 알고리즘은 같다.
    while begin <= n - m:
        if matched < m and H[begin + matched] == N[matched]:
            matched +=1
            if matched == m:                    #matchd가 m(비교할 문자열의 길이)라면 기준 문자열에서 부분일치를 찾은 것이므로
                ret.append(begin)               #기준 문자열의 부분일치 시작 지점을 기록한다.
        else:                                   #이후 알고리즘은 같다.
            if matched == 0:
                begin += 1
            else:
...
str_N = input()
x = kmp(str_H,str_N)
print(len(x))                                   #return된 list의 길이는 부분일치한 횟수와 같다.
for i in x:
    print(i+1,end=' ')                          #부분 일치했던 위치를 출력한다.(index는 0부터 시작이기 때문에 1을 더한다.)